...
def has_gap(k1, k2, min_gap=0.002):
    """判断 k1, k2 之间是否有缺口"""
    assert k2['dt'] > k1['dt']
    if k1['high'] < k2['low'] * (1 - min_gap) or k2['high'] < k1['low'] * (1 - min_gap):
        return True
    else:
        return False
...
class KlineAnalyze_old:
    ...
    def _update_bi_list(self):
        ...
        for fx in right_fx:
            ...
            if last_bi['fx_mark'] == fx['fx_mark']:
                if (last_bi['fx_mark'] == 'g' and last_bi['bi'] < bi['bi']) or (last_bi['fx_mark'] == 'd' and last_bi['bi'] > bi['bi']):
                    if self.verbose:
    ...
    def _update_xd_list(self):
        ...
        for xp in xd_p:
            ...
            if last_xd['fx_mark'] == xd['fx_mark']:
                if (last_xd['fx_mark'] == 'd' and last_xd['xd'] > xd['xd']) or (last_xd['fx_mark'] == 'g' and last_xd['xd'] < xd['xd']):
                    if self.verbose:
    ...
    def get_bi_fd(self, n=6):
        ...
        for i in range(len(points) - 1):
            ...
            # macd_power = self.calculate_macd_power(start_dt=p1['dt'],
            # end_dt=p2['dt'], mode="bi", direction=direction)
...
class KlineAnalyze:
    def __init__(self, kline, name="本级别", bi_mode="new", max_count=1000,
        ...
        # 根据输入K线初始化
        if isinstance(kline, pd.DataFrame):
            self.ohlc_data = kline
            columns = kline.columns.to_list()
            self.kline_raw = [{k: v for k, v in zip(columns, row)} for row in kline.values]
        else:
            self.kline_raw = kline
        
        if ('symbol' in self.ohlc_data.columns):
            self.symbol = elf.ohlc_data['symbol'].head(1).item()
        elif isinstance(self.ohlc_data.index, pd.MultiIndex):
            self.symbol = self.ohlc_data.index.get_level_values(level=1)[0]
        self.start_dt = self.kline_raw[0]['dt']
        self.end_dt = self.kline_raw[-1]['dt']
        self.latest_price = self.kline_raw[-1]['close']
